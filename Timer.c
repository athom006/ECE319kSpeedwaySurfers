...
void EdgeTriggered_Init(void(*task)(void)){
  GPIOA->POLARITY31_16 = (1<<((25-16)<<1));// PA25 rising
  GPIOA->POLARITY31_16 = (1<<((27-16)<<1)); // PA27 rising
  
  GPIOB->CPU_INT.ICLR = 0x02000000; // clear bit 25
  GPIOB->CPU_INT.ICLR = 0x08000000; // clear bit 27

  GPIOB->CPU_INT.IMASK = 0x02000000; // arm PB25
  GPIOB->CPU_INT.IMASK = 0x08000000; // arm PB27
...
void TimerG12_IntArm(void(*task)(void),  uint32_t period, uint32_t priority){
  TIMG12->GPRCM.RSTCTL = (uint32_t)0xB1000003;
  TIMG12->GPRCM.PWREN = (uint32_t)0x26000001;
  Clock_Delay(24); // time for TimerG12 to power up
  TIMG12->CLKSEL = 0x08; // bus clock
